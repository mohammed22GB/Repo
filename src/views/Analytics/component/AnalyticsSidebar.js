...
import { ressignTask } from "../AnalyticsApis";
...
const useStyles = makeStyles((theme) => ({
  ...
  sideHeadingBar: {
    ...
    justifyContent: "space-between",
    alignItems: "center",
    //borderTop: "1.2px solid darkgray",
    //borderBottom: "1px solid gainsboro",
    //boxShadow: "0px 20px 50px 0px lightgray inset",
    //backgroundColor: "#f8f8f8",
    height: 29,
    //cursor: "pointer",
    // "&:hover": {
...
const AnalyticsSidebar = ({
  ...
  const _convertBody = (content, wh) => {
    if (wh === "to-id") {
      if (
        ...
        //This toggles the Saved message indicator in rich text editor
  ...
  const _setTaskInfo = async (e, ppty) => {
    ...
    if (e?.selection === "removed" && ppty === "reassign") {
      if (assignees.length) {
        const filterUsers = assignees.filter((usr) => usr.id !== e.id);
        ...
        <>
          <div className="_heading">
            ...
            {data?.length ? (
              ...
              <div style={{ fontWeight: 300 }}>(None)</div>
            )}
            <IconButton
              aria-label="close"
            ...
            <div className={classes.sectionEntry} style={{ margin: 10 }}>
              <Typography gutterBottom className={classes.sectionTitle}>
                ...
                {/* <span
                  ...
                  onClick={() => setHideEmailBody(!hideEmailBody)}
                >
                  {hideEmailBody ? "Collapse" : "Expand"}
                </span> */}
                {hideEmailBody ? (
                  <>
                    <span
                      className="email-body-save-btn"
                      onClick={() => _convertBody(heldBody, "to-id")}
                    >
                      Save message
                    </span>
